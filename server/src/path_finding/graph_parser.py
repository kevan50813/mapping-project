...
class Parser:
    @classmethod
    async def create(cls, path, graph_name=None):
        self = Parser()
        ...
        await self.parse_nodes()
        await self.parse_rooms()
        await self.parse_pois()
    ...
    async def parse_nodes(self):
        ...
        await asyncio.gather(*[self.parse_node_feature(feature)
                             for feature in json_ways["features"]])

    async def parse_node_feature(self, feature):
        prevId = -1

        for points in feature["geometry"]["coordinates"]:
            p = (points[0], points[1])
            # is p already in self.nodes ?
            # else add it, then do the rest, means we can async :)
    ...
    async def parse_rooms(self):
        ...
        await asyncio.gather(*[self.parse_room(node, json_rooms)
                               for node in self.nodes])

    async def parse_room(self, node, json_rooms):
    ...
    async def parse_pois(self):
        ...
        await asyncio.gather(*[self.parse_poi(poi, json_rooms)
                               for poi in json_poi["features"]])

    async def parse_poi(self, poi, json_rooms):
        id = len(self.pois)
